...
typedef enum
{
   ...
   /* Linear PCM Encoded Data that can be procoessed by the Bluetooth   */
...
typedef struct _tagSCO_Data_Format_t
{
   ...
   /* The following enumerated type specifies the retransmition modes   */
...
#define SCO_PACKET_SYNCHRONOUS_CONNECTION_TYPE_DEFAULT            ( HCI_PACKET_SYNCHRONOUS_CONNECTION_TYPE_HV1                   |   \
   ...
   /* The TransmitBandwidth and ReceiveBandwidth are in in octets per   */
   ...
   /* be greate than 3.  The VoiceSettings is defined in the            */
...
#define SCO_DISCONNECT_EVENT_SIZE                       (sizeof(SCO_Disconnect_Event_t))
   ...
   /* * NOTE * DataBuffer is NOT a Pointer to the data, it is the actual*/
   /*          Data itself.  A Utility MACRO exists to aid in the       */
   /*          calculation of the total size (in bytes) of the entire   */
   /*          structure (including the SCO Data).                      */
typedef struct _tagSCO_Data_Indication_Event_t
{
   ...
   /* in calculating the total size of the SCO_Data_Indication_Event_t  */
   /* structure given the length (in Bytes) of the SCO Data that is     */
   /* contained the structure.  This MACRO accepts as it's single       */
   /* parameter, the Length (specified in Bytes) of the SCO Data that   */
   /* is/is to be contained in the SCO_Data_Indication_Event_t          */
   /* structure.                                                        */
#define SCO_DATA_INDICATION_EVENT_SIZE(_x)              (sizeof(SCO_Data_Indication_Event_t) - sizeof(Byte_t) + ((unsigned int)(_x)))

   /* The following type Declaration represents the Data that is        */
   /* associated with the SCO Transmit Data Empty Event.  This Event    */
   /* is dispatched when the Transmit Queue is empty, but ONLY after    */
   /* the Transmit Queue was COMPLETELY full.  See the SCO_Queue_Data() */
...
BTPSAPI_DECLARATION int BTPSAPI SCO_Register_Synchronous_Connect_Request_Callback(unsigned int BluetoothStackID, SCO_Connect_Request_Callback_t SCO_Connect_Request_Callback, unsigned long CallbackParameter);
   ...
   /* the caller can remove the SCO Connection Reuqest Callback from the*/
...
BTPSAPI_DECLARATION int BTPSAPI SCO_Un_Register_Callback(unsigned int BluetoothStackID, unsigned int SCOCallbackID);
   ...
   /* specifed Connection Callback with the Connection Result.  A       */
...
BTPSAPI_DECLARATION int BTPSAPI SCO_Setup_Synchronous_Connection(unsigned int BluetoothStackID, BD_ADDR_t BD_ADDR, SCO_Synchronous_Connection_Info_t *SynchronousConnectionInfo, SCO_Connection_Callback_t SCO_Connection_Callback, unsigned long CallbackParameter);
   ...
   /* on the specified SCO Connection.   Note that there must already   */
   /* be a Connection to the specified Bluetooth Device for this        */
   /* function to succeed (NON SCO Connection already - ACL).  This     */
   /* function will return a non-zero, positive value on success which  */
   /* indicates the SCO Connection ID of the specified Connection Link. */
   /* If this function returns a negative value then a SCO Connection   */
   /* was NOT able to be established with the specified Bluetooth       */
   /* Device (based upon the BD_ADDR_t specified.  The caller can close */
   /* the SCO Connection when finished by passing the return value from */
   /* this function (if successful) to the SCO_Close_Connection()       */
   /* function.  If this function returns a positive value, then a SCO  */
   /* Connection has been requested with the specified Board Address.   */
   /* Note that this does NOT mean that the SCO Connection has been     */
   /* established in the case of a SCO Accept.  This information is     */
   /* returned in the specifed SCO Connection Callback with the         */
   /* Connection Result.  A positive return value represents the SCO    */
   /* Connection ID of the SCO Connection that is in progress.          */
   /* ** NOTE ** This function is deprecated, as it only supports SCO   */
...
BTPSAPI_DECLARATION int BTPSAPI SCO_Close_Connection(unsigned int BluetoothStackID, unsigned int SCOConnectionID, unsigned int Disconnect_Status);
   ...
   /* Accept.  This information is returned in specifed SCO/eSCO        */
   /* Connection Callback with the Connection Result.                   */
   /* ** NOTE **    This function can ONLY be called in the context of a*/
   /*               SCO/eSCO Connection Request Callback.  If this      */
   /*               function is called from any other context it will   */
   /*               fail.  This means that a SCO/eSCO Connection can    */
   /*               ONLY be accepted/rejected during a SCO/eSCO         */
   /*               Connection Request Callback.                        */
   /* ** NOTE **    The Connection can ONLY be accepted/rejected during */
   /*               the SCO/eSCO Connection Request Callback.  If the   */
   /*               SCO_Accept_Synchronous_Connection() function is not */
   /*               called during this callback then there is NO way to */
   /*               Accept/Reject the SCO/eSCO Connection Request and   */
   /*               the Connection Request will timeout (on the         */
   /*               originators end).                                   */
BTPSAPI_DECLARATION int BTPSAPI SCO_Accept_Synchronous_Connection(unsigned int BluetoothStackID, unsigned int SCOConnectionID, SCO_Synchronous_Connection_Info_t *SynchronousConnectionInfo, unsigned int RejectReason, SCO_Connection_Callback_t SCO_Connection_Callback, unsigned long CallbackParameter);
   ...
   /* a SCO Connection Request.  This function CAN ONLY be called in the*/
   /* Context of a SCO Connection Request Callback.  The first parameter*/
   /* of this function is obtained vaia successful call to the          */
   /* BSC_Initialize() function, and the second parameter is obtained   */
   /* via the SCO_Connection_ID member of the SCO_Connect_Request Data  */
   /* member of the SCO Connect Request Event Data.  This Data is       */
   ...
   /* is to be installed for the accepted SCO Connection.  These        */
   /* two parameters are ignored if the SCO Connection is being         */
   /* rejected, however they MUST be valid if the SCO Connection is     */
   /* being accepted.  This Callback Function (and specified Callback   */
   /* Parameter) will be used when any SCO Event occurs on the          */
   /* accepted SCO Connection (if accepted).  If this function returns  */
   /* zero, then the SCO Connection has been accepted or rejected.      */
   /* Note that this does NOT mean that the SCO Connection has been     */
   /* established in the case of a SCO Accept.  This information is     */
   /* returned in specifed SCO Connection Callback with the Connection  */
   /* Result.                                                           */
   /* ** NOTE ** This function can ONLY be called in the context of a   */
...
BTPSAPI_DECLARATION int BTPSAPI SCO_Accept_Connection(unsigned int BluetoothStackID, unsigned int SCOConnectionID, unsigned int RejectReason, SCO_Connection_Callback_t SCO_Connection_Callback, unsigned long CallbackParameter);
   ...
   /* The following function is used to modify an existing synchrounous */
   ...
   /* in the status of etSynchronous_Connection_Changed_Event returnd   */
   /* from the SCO_Connection_Callback passed in durring                */
...
BTPSAPI_DECLARATION int BTPSAPI SCO_Modify_Synchronous_Connection(unsigned int BluetoothStackID, unsigned int SCOConnectionID, Word_t MaxLatency, SCO_Retransmission_Effort_t RetransmissionEffort);
   ...
   /* the the specified SCO/eSCO Connection.  This function accepts as  */
...
BTPSAPI_DECLARATION int BTPSAPI SCO_Purge_Buffer(unsigned int BluetoothStackID, unsigned int SCOConnectionID, unsigned int PurgeBufferMask);
   ...
   /*          data again via the the                                   */
   /*          etSCO_Transmit_Buffer_Empty_Indication SCO/eSCO Event.   */
   /*          This will allow the user a mechanism to know when the    */
   /*          Transmit Buffer is empty so that more data can be queued */
   /*          for sending.                                             */
