...
BTPSAPI_DECLARATION int BTPSAPI GAP_LE_Parse_Scan_Response_Data(Scan_Response_Data_t *Scan_Response_Data, GAP_LE_Advertising_Data_t *GAP_LE_Advertising_Data);
   ...
   /* GAP_LE_Set_Advertising_Data() with Type set to dtScanResponse) and*/
   /* a boolean which specifies whether to enable advertising (TRUE) or */
   /* disable advertising (FALSE).  The next parameter is a pointer to a*/
   /* structure containing advertising parameters.  The next parameter  */
   /* is a pointer to a structure specifying the connectability mode and*/
   /* connectability mode parameters to use.  The final two parameters  */
   /* are the GAP LE event callback and callback paramater to use when  */
   /* receiving events caused by advertising (like connection events).  */
   /* This function will return zero on success, or a negative return   */
   /* error code if there was an error.                                 */
