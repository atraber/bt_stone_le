...
#define L2CA_TIMEOUT_INDICATION_SIZE                    (sizeof(L2CA_Timeout_Indication_t))
   ...
   /* The following structure represents the structure of the L2CAP     */
   /* Echo Confirmation Event Data.  The only detail to note is that the*/
   /* Variable_Data field is an array of Bytes and NOT a Pointer to an  */
   /* array of Bytes.  This is very important, because this mechanism   */
   /* will allow an arbitrary memory buffer to be typecast to this      */
   /* structure and all elements will be accessible in the same memory  */
   /* block (i.e. NO other Pointer operation is required).  The side    */
   /* effect of this is that when the memory for a                      */
   /* L2CA_Echo_Confirmation_t structure is allocated, the allocated    */
   /* size that is required will need to be calculated instead of just  */
   /* using the sizeof() operator.  After this is completed, the data   */
   /* elements in the Packet Data array can be accessed by simply Array */
   /* Logic, aiding code readability.  It might appear confusing to the */
   /* user because array elements greater than zero will be indexed,    */
   /* however, as long as the programmer is aware of this design        */
   /* decision, the code should be much more simple to read.  MACRO's   */
   /* and Definitions will be provided following this structure         */
   /* definition to alleviate the programmer from having to remember    */
   /* (or even determine) the correct formula for determining the       */
   /* number of bytes required for a certain data length when           */
...
typedef struct _tagL2CA_Echo_Confirmation_t
{
   ...
   /* L2CA Echo Confirmation Data Structure based upon the number of    */
   /* Echo Data Bytes associated with the Echo Request. The first       */
   /* parameter to this MACRO is the number of Bytes of Echo Data       */
   /* that are to be part of the L2CA Echo Request.                     */
#define L2CA_ECHO_CONFIRMATION_SIZE(_x)                 (BTPS_STRUCTURE_OFFSET(L2CA_Echo_Confirmation_t, Variable_Data) + (unsigned int)(_x))

   /* The following structure represents the structure of the L2CAP     */
   /* Information Confirmation Event Data.  The only detail to note is  */
   /* that the Variable_Data field is an array of Bytes and NOT a       */
   /* Pointer to an array of Bytes.  This is very important, because    */
   /* this mechanism will allow an arbitrary memory buffer to be        */
   ...
   /* L2CA_Information_Confirmation_t structure is allocated, the       */
   /* allocated size that is required will need to be calculated        */
   /* instead of just using the sizeof() operator.  After this is       */
   /* completed, the data elements in the Packet Data array can be      */
   /* accessed by simply Array Logic, aiding code readability.  It might*/
   /* appear confusing to the user because array elements greater       */
   /* than zero will be indexed, however, as long as the programmer is  */
   /* aware of this design decision, the code should be much more       */
   /* simple to read.  MACRO's and Definitions will be provided         */
   /* following this structure definition to alleviate the programmer   */
   /* from having to remember (or even determine) the correct formula   */
   /* for determining the number of bytes required for a certain data   */
   /* length when allocating memory of the correct size.                */
typedef struct _tagL2CA_Information_Confirmation_t
{
   ...
   /* The following MACRO is a utility MACRO that exists to aid code    */
   /* readability to Determine the size (in Bytes) of an L2CA           */
   /* Information Confirmation Data Structure based upon the number of  */
   /* Information Data Bytes associated with the Information Request.   */
   /* The first parameter to this MACRO is the number of Bytes of       */
   /* Information Data that are the response to the L2CA Information    */
   /* Request.                                                          */
#define L2CA_INFORMATION_CONFIRMATION_SIZE(_x)          (BTPS_STRUCTURE_OFFSET(L2CA_Information_Confirmation_t, Variable_Data) + (unsigned int)(_x))
...
#define L2CA_INFORMATION_CONFIRMATION_SET_CONNECTIONLESS_MTU_DATA(_x, _y)   (((Word_t *)(((L2CA_Information_Confirmation_t *)(_x))->Variable_Data))[0] = (Word_t)(_y))
   ...
   /* The following structure represents the structure of the L2CAP     */
   /* Connectionless Data Indication Event Data.  The only detail to    */
   /* note is that the Variable_Data field is an array of Bytes and NOT */
   /* a Pointer to an array of Bytes.  This is very important, because  */
   /* this mechanism will allow an arbitrary memory buffer to be        */
   /* typecast to this structure and all elements will be accessible in */
   /* the same memory block (i.e. NO other Pointer operation is         */
   /* required).  The side effect of this is that when the memory for a */
   /* L2CA_Group_Data_Indication_t structure is allocated, the allocated*/
   /* size that is required will need to be calculated instead of just  */
   /* using the sizeof() operator.  After this is completed, the data   */
   /* elements in the Packet Data array can be accessed by simply Array */
   /* Logic, aiding code readability.  It might appear confusing to the */
...
typedef struct _tagL2CA_Group_Data_Indication_t
{
   ...
   /* The following MACRO is a utility MACRO that exists to aid code    */
   /* readability to Determine the size (in Bytes) of an                */
   /* L2CA Broadcast Indication Data Structure based upon the number of */
   /* Data Bytes associated with the Data Indication.  The first        */
   /* parameter to this MACRO is the number of Bytes of L2CAP Data      */
   /* that is to be part of the L2CA Data Indication.                   */
#define L2CA_GROUP_DATA_INDICATION_SIZE(_x)           (BTPS_STRUCTURE_OFFSET(L2CA_Group_Data_Indication_t, Variable_Data) + (unsigned int)(_x))

   /* The following structure represents the structure of the L2CAP     */
   /* Data Indication Event Data.  The only detail to note is that the  */
   /* Variable_Data field is an array of Bytes and NOT a Pointer to an  */
   /* array of Bytes.  This is very important, because this mechanism   */
   /* will allow an arbitrary memory buffer to be typecast to this      */
   /* structure and all elements will be accessible in the same memory  */
   /* block (i.e. NO other Pointer operation is required).  The side    */
   /* effect of this is that when the memory for a                      */
   /* L2CA_Data_Indication_t structure is allocated, the allocated      */
   /* size that is required will need to be calculated instead of just  */
   /* using the sizeof() operator.  After this is completed, the data   */
   /* elements in the Packet Data array can be accessed by simply Array */
   /* Logic, aiding code readability.  It might appear confusing to the */
...
typedef struct _tagL2CA_Data_Indication_t
{
   ...
   /* The following MACRO is a utility MACRO that exists to aid code    */
   /* readability to Determine the size (in Bytes) of an                */
   /* L2CA Data Indication Data Structure based upon the number of Data */
   /* Data Bytes associated with the Data Indication.  The first        */
   /* parameter to this MACRO is the number of Bytes of L2CAP Data      */
   /* that are to be part of the L2CA Data Indication.                  */
#define L2CA_DATA_INDICATION_SIZE(_x)                   (BTPS_STRUCTURE_OFFSET(L2CA_Data_Indication_t, Variable_Data) + (unsigned int)(_x))
...
#define L2CA_FIXED_CHANNEL_DISCONNECT_INDICATION_SIZE      (sizeof(L2CA_Fixed_Channel_Disconnect_Indication_t))
   ...
   /* The following structure represents the structure of the L2CAP     */
   /* Fixed Channel Data Indication Event Data.  The only detail to note*/
   /* is that the Variable_Data field is an array of Bytes and NOT a    */
   /* Pointer to an array of Bytes.  This is very important, because    */
   /* this mechanism will allow an arbitrary memory buffer to be        */
   /* typecast to this structure and all elements will be accessible in */
   /* the same memory block (i.e. NO other Pointer operation is         */
   /* required).  The side effect of this is that when the memory for a */
   /* L2CA_Fixed_Channel_Data_Indication_t structure is allocated, the  */
   /* allocated size that is required will need to be calculated instead*/
   /* of just using the sizeof() operator.  After this is completed, the*/
   /* data elements in the Packet Data array can be accessed by simply  */
   /* Array Logic, aiding code readability.  It might appear confusing  */
   /* to the user because array elements greater than zero will be      */
   /* indexed, however, as long as the programmer is aware of this      */
   /* design decision, the code should be much more simple to read.     */
   /* MACRO's and Definitions will be provided following this structure */
   /* definition to alleviate the programmer from having to remember (or*/
   /* even determine) the correct formula for determining the number of */
   /* bytes required for a certain data length when allocating memory of*/
   /* the correct size.                                                 */
typedef struct _tagL2CA_Fixed_Channel_Data_Indication_t
{
   ...
   /* The following MACRO is a utility MACRO that exists to aid code    */
   /* readability to Determine the size (in Bytes) of an L2CA Fixed     */
   /* Channel Data Indication Data Structure based upon the number of   */
   /* Data Data Bytes associated with the Fixed Channel Data Indication.*/
   /* The first parameter to this MACRO is the number of Bytes of L2CAP */
   /* Data that are to be part of the L2CA Fixed Channel Data           */
   /* Indication.                                                       */
#define L2CA_FIXED_CHANNEL_DATA_INDICATION_SIZE(_x)         (BTPS_STRUCTURE_OFFSET(L2CA_Fixed_Channel_Data_Indication_t, Variable_Data) + (unsigned int)(_x))
...
BTPSAPI_DECLARATION int BTPSAPI L2CA_Data_Write(unsigned int BluetoothStackID, Word_t LCID, Word_t Data_Length, Byte_t *Data);
   ...
   /*          the etChannel_Buffer_Empty_Indication Event before trying*/
...
BTPSAPI_DECLARATION long BTPSAPI L2CA_Enhanced_Data_Write(unsigned int BluetoothStackID, Word_t LCID, L2CA_Queueing_Parameters_t *QueueingParameters, Word_t Data_Length, Byte_t *Data);
   ...
   /* The input parameters to this function are the Bluetooth Stack ID  */
   /* of the Bluetooth Stack that the second parameter identifies the   */
   /* address of the remote device that is to receive the data.  The    */
   /* third parameter identifies the Fixed Channel on which the data is */
   /* to be sent.  The Length of the Data to send and a pointer to the  */
