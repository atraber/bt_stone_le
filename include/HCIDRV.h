...
#define HCI_DRIVER_ERROR_UNIMPLEMENTED          (-11)   /* Error that denotes */
   ...
   /* called whenever a complete HCI Packet has been received by the    */
   /* HCI Driver that was opened with the specified Driver ID.  This    */
   ...
   /* to use the contents of the HCI Packet ONLY in the context of      */
   ...
   /* installed for different Callback Types, then the function IS NOT  */
   /* guaranteed to be NOT be called simultaneously.  The function is   */
   /* only guaranteed to be called serially for the context in which    */
   /* the Callback has been installed.  This means that if the same     */
   /* Callback function is installed for both ACL Data and HCI Event    */
   /* Packets, then the function COULD be called simultaneously for     */
   /* both Packet Types, but NOT simultaneously for HCI Event Packets   */
   /* OR ACL Data Packets.  It should also be noted that this function  */
   ...
   /*            can only be satisfied by Receiving HCI Data Packets.   */
   /*            A Deadlock WILL occur because NO Other Receive HCI     */
...
typedef void (BTPSAPI *HCI_DriverCallback_t)(unsigned int HCIDriverID, HCI_Packet_t *HCIPacket, unsigned long CallbackParameter);
   ...
   /* guaranteed to be NOT be called simultaneously.  The function is   */
   /* only guaranteed to be called serially for the context in which    */
   /* the Callback has been installed.  It should also be noted that    */
   /* this function is called in the Thread Context of a Thread that the*/
   /* User does NOT own.  Therefore, processing in this function should */
   /* be as efficient as possible (this argument holds anyway because   */
   /* Packet Processing (Sending/Receiving) will be suspended while     */
...
BTPSAPI_DECLARATION int BTPSAPI HCI_OpenDriver(HCI_DriverInformation_t *DriverInformation);
   ...
   /* currently active HCI Driver.  The The Input parameter to this     */
   /* function MUST have been acquired by a successful call to          */
   /* HCI_OpenDriver().  This function accepts the HCI Driver ID of the */
   /* driver to reconfigured, followed by a BOOLEAN which specifies     */
   /* whether or not any internal HCI Driver state machines should be   */
   /* reset (for example BCSP and/or Packet building state machines).   */
...
BTPSAPI_DECLARATION void BTPSAPI HCI_ProcessHCIDriverInformation(unsigned int HCIDriverID);
   ...
   /* Packet is received.  The first pararmeter is obtained by a        */
   /* successful call to the HCI_OpenDriver() function.  The second     */
   /* parameter is a pointer to a function that is to be called when    */
   /* an HCI Event Packet is received on the specified HCI Packet       */
   /* Driver (specified by the first parameter).  The CallbackParameter */
   /* Parameter is a User defined value that will passed to the         */
   /* Callback Parameter when it is called.  The return value of this   */
   /* function will be a non-zero, non-negative value on success or a   */
   /* negative return code if there was a failure.  On success the      */
   /* value represents an HCI Callback ID that can be used in a call    */
   /* to the HCI_UnRegisterCallback() function to remove the Callback   */
   /* from the HCI Packet Driver System.                                */
...
BTPSAPI_DECLARATION int BTPSAPI HCI_RegisterSCODataCallback(unsigned int HCIDriverID, HCI_DriverCallback_t SCODataCallback, unsigned long CallbackParameter);
   ...
   /* Callback Funtion to delete.  This value is a Callback ID that     */
   /* was returned when a specified Callback was Registered.  Callbacks */
   /* can only be registered with the following functions:              */
   /*   - The HCI_RegisterEventCallback() function.                     */
   /*   - The HCI_RegisterACLDataCallback() function.                   */
   /*   - The HCI_RegisterSCODataCallback() function.                   */
   /*   - The HCI_RegisterDebugCallback() function.                     */
   /* After this function is called, the previously installed Callback  */
   /* function will NOT be called when HCI Packet Data is received      */
   /* (unless the function has been registered more than once, in which */
   /* case the function will have another Callback ID).                 */
BTPSAPI_DECLARATION void BTPSAPI HCI_UnRegisterCallback(unsigned int HCIDriverID, unsigned int CallbackID);
   ...
   /* The following function is responsible for actually sending an     */
   /* HCI Packet through the Opened HCI Driver.  This function accepts  */
   /* as input the HCI Driver to send the specified HCI Packet through  */
   /* and the HCI Packet to send.  The last two parameters are valid    */
   /* ONLY for HCI Command Packets and are ignored for the other types  */
   /* of HCI Packets that are sent through this function.  These        */
   /* parameters provide a means for the caller to specify a Callback   */
   /* function that will be called when an HCI Event Response (only for */
   /* the specified HCI Command Type) is received.  This will allow     */
   /* the caller a means of providing the synchronous sending of HCI    */
   /* Command Data (possibly).  This mechanism is not required, and     */
   /* if the Callback Function specified is NULL, then this mechanism   */
   /* is not utilized. This function returns a non-zero, non-negative   */
   /* value on success, or a negative return code if there was an       */
   /* error (see error codes above).  A successful call to this         */
   /* function will yield an HCI Packet ID that can be passed to the    */
   /* HCI_CancelPacket() function to possibly Cancel the Packet from    */
   /* being sent (at some future time).                                 */
   /* * NOTE * This function does NOT immediately send the Data to      */
   /*          the specified HCI Driver Device.  The HCI Driver can     */
   /*          possibly Queue the Packet to send at a future time       */
   /*          (based upon the state of the Bluetooth Device and the    */
   /*          type of HCI Packet Data to be sent).  An example where   */
   /*          the HCI Packet Driver would Queue an HCI Packet would be */
   /*          an HCI Command Packet if the Bluetooth Device informed   */
   /*          the HCI Packet Driver that it can no longer accept       */
   /*          HCI Command Packets.  The HCI Command would then be      */
   /*          De-Queued when the Bluetooth Device informed the HCI     */
   /*          Packet Driver that it can accept HCI Command Data.       */
...
BTPSAPI_DECLARATION int BTPSAPI HCI_ChangeSCOConfigurationHandles(unsigned int HCIDriverID, unsigned int NumberHandles, Word_t *ConnectionHandleList);
   ...
   /* HCI_OpenDriver() function.  The second parameter is a pointer to  */
   /* a function that is to be called when an HCI Packet (of any type)  */
   /* is Sent OR Received on the specified HCI Packet Driver (specified */
   /* by the first parameter).  The CallbackParameter Parameter is a    */
   /* User defined value that will passed to the Callback Parameter when*/
   /* it is called.  The return value of this function will be a        */
   /* non-zero, non-negative value on success or a negative return code */
   /* if there was a failure.  On success the value represents an HCI   */
   /* Callback ID that can be used in a call to the                     */
   /* HCI_UnRegisterCallback() function to remove the Debugging Callback*/
   /* from the from the HCI Packet Driver System.                       */
