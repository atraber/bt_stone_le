...
#include "HRDWCFG.h"              /* SS1 MSP430 Hardware Configuration Consts.*/
   ...
   if((Used) && (Free) && (MaxFree) && (HeapHead))
   {
      ...
      *MaxFree      = 0;
      ...
      do
      {
         ...
         BTPS_OutputMessage("(%u %u)%c ", HeapInfoPtr->PrevSize, (HeapInfoPtr->Size & SEGMENT_SIZE_BITMASK), (HeapInfoPtr->Size & SEGMENT_ALLOCATED_BITMASK)?'U':'F');
...
static void HeapInit(void)
{
   ...
   if((BTPS_STRUCTURE_OFFSET(HeapInfo_t, Data) % ALIGNMENT_SIZE) == 0)
   {
      ...
      if(HeapSize <= SEGMENT_SIZE_BITMASK)
      {
         ...
         DBG_MSG(DBG_ZONE_BTPSKRNL, ("%s Head %p Tail %p Size %x %x\r\n", __FUNCTION__, HeapHead, HeapTail, HeapSize, (HeapTail - HeapHead)));
      }
   }
}
...
static void *_Malloc(unsigned long Size)
{
   ...
   DBG_MSG(DBG_ZONE_BTPSKRNL, ("MALLOC SIZE %u\r\n", Size));
   ...
   if((Size >= HEAP_INFO_DATA_SIZE(1)) && (!(Size & SEGMENT_ALLOCATED_BITMASK)))
   {
      ...
      if(HeapHead)
      {
         ...
         if(Size >= LARGE_SIZE)
         {
            DBG_MSG(DBG_ZONE_BTPSKRNL, ("LARGE %p %p %u %p.\r\n", HeapTail, HeapHead, HeapHead->PrevSize, ((Alignment_t *)HeapTail) - HeapHead->PrevSize));
            HeapInfoPtr = (HeapInfo_t *)(((Alignment_t *)HeapTail) - HeapHead->PrevSize);
            DBG_MSG(DBG_ZONE_BTPSKRNL, ("After 1.\r\n"));
         }
         else
            HeapInfoPtr = HeapHead;
         ...
         if((HeapInfoPtr != HeapTail) && !(HeapInfoPtr->Size & SEGMENT_ALLOCATED_BITMASK) && (HeapInfoPtr->Size >= Size))
         {
            ...
            if(HeapInfoPtr->Size >= (Size + HEAP_INFO_DATA_SIZE(MINIMUM_MEMORY_SIZE)))
            {
               ...
               if(Size >= LARGE_SIZE)
               {
                  ...
                  DBG_MSG(DBG_ZONE_BTPSKRNL, ("LARGE SPLIT (%p %p).\r\n", HeapInfoPtr, TmpInfoPtr));
               ...
               else
               {
                  ...
                  DBG_MSG(DBG_ZONE_BTPSKRNL, ("SMALL SPLIT (%p %p).\r\n", HeapInfoPtr, TmpInfoPtr));
            ...
            {
               DBG_MSG(DBG_ZONE_BTPSKRNL, ("NO SPLIT.\r\n"));
   ...
   if((MemoryPtr) && (HeapHead) && (((HeapInfo_t *)MemoryPtr) >= HeapHead) && (((HeapInfo_t *)MemoryPtr) < HeapTail))
   {
      ...
      if(HeapInfoPtr->Size & SEGMENT_ALLOCATED_BITMASK)
      {
         ...
         if(HeapInfoPtr != HeapHead)
         {
            ...
            {
               DBG_MSG(DBG_ZONE_BTPSKRNL, ("PREV COMBINE.\r\n"));
         ...
         else
         {
            ...
            {
               DBG_MSG(DBG_ZONE_BTPSKRNL, ("NEXT COMBINE.\r\n"));
