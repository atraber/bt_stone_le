...
#define HCITR_ERROR_WRITING_TO_PORT          (-3)       /* Denotes that an    */
   ...
   /* member) to specify that the UART should assert/de-assert the      */
   /* signal to send/receive characters.  The ReconfigureData member    */
   /* will simply be a NON-NULL pointer (it will not be dereferenced) to*/
   /* enable flow, and a NULL pointer to disable flow.                  */
#define HCI_COMM_DRIVER_DISABLE_UART_TX_RX   (HCI_COMM_DRIVER_RECONFIGURE_DATA_COMMAND_CHANGE_PARAMETERS + 1)
