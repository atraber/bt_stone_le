...
#define DEBUG_ENABLED//***
   ...
   /* The following defines the maximum number of bytes that a debug    */
   /* message can be.  All debug strings are truncated to this value    */
   /* minus one to allow for a NULL terminator.                         */
...
int BTPSAPI BTPS_QueryMemoryUsage(unsigned int *Used, unsigned int *Free, unsigned int *MaxFree);
   ...
   typedef int (BTPSAPI *PFN_BTPS_QueryMemoryUsage_t)(unsigned int *Used, unsigned int *Free, unsigned int *MaxFree);
#endif
